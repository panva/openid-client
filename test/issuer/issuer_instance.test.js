...
describe('Issuer', () => {
  ...
  describe('key storage behavior (using deprecated .key)', function () {
    it('requires jwks_uri to be configured', function () {
      const issuer = new Issuer();
    ...
    it('multiple keys cannot match the JWT header', function () {
      sinon.stub(LRU.prototype, 'get').returns(undefined);
      const { kid } = this.keystore.get({ kty: 'RSA' });
      return this.keystore.generate('RSA', undefined, { kid }).then(() => {
        nock('https://op.example.com')
        ...
        return this.issuer.key({ alg: 'RS256', kid }).then(fail, (err) => {
          expect(nock.isDone()).to.be.true;
          expect(err.message).to.equal(`multiple matching keys found in issuer's jwks_uri for key parameters {"kid":"${kid}","alg":"RS256"}, kid must be provided in this case`);
        });
      });
    });
