...
<body onload="javascript:document.forms[0].submit()">
  ...
  userinfo(accessToken, options) {
    ...
    return http[verb](this.issuer.userinfo_endpoint, this.issuer.httpOptions(httpOptions))
  ...
  fetchDistributedClaims(claims, accessTokens) {
    ...
    return Promise.all(_.map(distributedSources, (def, sourceName) => {
      ...
      return http.get(def.endpoint, this.issuer.httpOptions(opts))
  ...
  authenticatedPost(endpoint, httpOptions) {
    return Promise.resolve(this.authFor(endpoint))
      .then(auth => http.post(this.issuer[`${endpoint}_endpoint`], this.issuer.httpOptions(_.merge(httpOptions, auth)))
      .catch(errorHandler));
  }
  ...
  static register(properties, opts) {
    ...
    return http.post(this.issuer.registration_endpoint, this.issuer.httpOptions({
      headers,
  ...
  static fromUri(uri, token) {
    return http.get(uri, this.issuer.httpOptions({
