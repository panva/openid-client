...
<body onload="javascript:document.forms[0].submit()">
  ...
  validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
    ...
    if (payload.iss !== undefined) {
      if (this.isAzureADv2Multitenant === true && this.issuer.issuer === 'https://login.microsoftonline.com/{tenantid}/v2.0') {
        if (this.azureADv2ValidTenantIds !== undefined && this.azureADv2ValidTenantIds.length > 0) {
          assert(this.azureADv2ValidTenantIds.includes(payload.tid), `tid (${payload.tid}) is not included in allowed tenant ids`);
        }
