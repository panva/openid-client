...
<body onload="javascript:document.forms[0].submit()">
  ...
  async decryptJWE(jwe, expectedAlg, expectedEnc = 'A128CBC-HS256') {
    ...
    if (expectedAlg.match(/^(?:RSA|ECDH)/)) {
      const keystore = await instance(this).get('keystore');
      // eslint-disable-next-line no-restricted-syntax
      for (const { keyObject: key } of keystore.all({ ...jose.decodeProtectedHeader(jwe), use: 'enc' })) {
  ...
  async validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
    ...
    if (returnedBy === 'authorization') {
      ...
      if (payload.s_hash) {
        ...
        try {
          tokenHash.validate({ claim: 's_hash', source: 'state' }, payload.s_hash, state, header.alg, key.jwk && key.jwk.crv);
        } catch (err) {
    ...
    if (tokenSet.access_token && payload.at_hash !== undefined) {
      try {
        tokenHash.validate({ claim: 'at_hash', source: 'access_token' }, payload.at_hash, tokenSet.access_token, header.alg, key.jwk && key.jwk.crv);
      } catch (err) {
        throw new RPError({ message: err.message, jwt: idToken });
      }
    }
    ...
    if (tokenSet.code && payload.c_hash !== undefined) {
      try {
        tokenHash.validate({ claim: 'c_hash', source: 'code' }, payload.c_hash, tokenSet.code, header.alg, key.jwk && key.jwk.crv);
      } catch (err) {
        throw new RPError({ message: err.message, jwt: idToken });
      }
    }
  ...
  async validateJWT(jwt, expectedAlg, required = ['iss', 'sub', 'aud', 'exp', 'iat']) {
    ...
    if (isSelfIssued) {
      ...
      keys = await this.issuer.queryKeyStore({ ...header, use: 'sig' });
    }
    ...
    for (const key of keys) {
      ...
      const verified = await jose.compactVerify(
        jwt,
        key instanceof Uint8Array ? key : key.keyObject,
      ).catch(() => {});
      if (verified) {
        return {
          payload,
  ...
  async requestObject(requestObject = {}, {
    ...
    if (fields.alg.match(/^(RSA|ECDH)/)) {
      [key] = await this.issuer.queryKeyStore({ alg: fields.alg, use: 'enc' }, { allowMulti: true });
    } else {
