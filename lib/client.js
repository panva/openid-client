...
const debug = require('debug')('openidclient');
...
<body onload="javascript:document.forms[0].submit()">
  ...
  authorizationCallback(redirectUri, parameters, checks = {}) {
    this.issuer.logger({ message: 'SSO begin authorizationCallback' });
    ...
    if (params.id_token) {
      this.issuer.logger({ message: 'SSO OpenId Client authorizationCallback', id_token: params.id_token });
      promise = Promise.resolve(new TokenSet(params))
        .then((tokenset) => {
          this.issuer.logger({ message: 'SSO begin decryptIdToken' });
          return this.decryptIdToken(tokenset);
        })
        .then((tokenset) => {
          this.issuer.logger({ message: 'SSO begin validateIdToken' });
   ...
   */
  grant(body) {
    this.issuer.logger({ message: 'SSO OpenId Client begin grant' });
    assertIssuerConfiguration(this.issuer, 'token_endpoint');
    return this.authenticatedPost('token', { body: _.omitBy(body, _.isUndefined) })
      .then(expectResponseWithBody(200))
      .then((response) => {
        this.issuer.logger({ message: 'SSO OpenId Client grant', response: response.body });
        return new TokenSet(JSON.parse(response.body));
      })
      .catch((err) => {
        this.issuer.logger({ message: 'SSO OpenId Client error', err });
        throw err;
      });
  }
