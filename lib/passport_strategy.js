...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(req, options) {
  ...
  try {
    ...
    const passReq = this._params.passReqToCallback;
    const loadUserinfo = this._verify.length > (passReq ? 3 : 2) &&
    ...
    callback.then((result) => {
      ...
      if (passReq) {
        verifyArgs.unshift(req);
      }

      if (loadUserinfo) {
        verifyArgs.push(result.userinfo);
      }

      verifyArgs.push(verified.bind(this));

      this._verify.apply(null, verifyArgs);
    }).catch((error) => {
